  ...
  const newsItems = [
    {
      ...
      readTime: '8 Min Read',
      excerpt: 'How a return to traditional debate formats and architectural governance can heal the fractured landscape of modern political engagement.',
    ...
    },
    {
      ...
      readTime: '5 Min Read',
      excerpt: 'An analysis of our newly released strategy for municipal growth and sustainable urban development in the tri-state area.',
    ...
    {
      id: 3,
      title: 'Sovereign Editorial Endorsed by National Industry Board',
      category: 'Press Release',
      date: 'October 15, 2024',
      readTime: '3 Min Read',
      excerpt: 'The National Industry Board announces support for our economic reform platform and steady leadership approach.',
      ...
      <main className="pt-20">
        ...
        <section className="py-16 bg-surface-container-low">
          <div className="max-w-screen-2xl mx-auto px-6">
            <h1 className="font-headline text-5xl md:text-7xl mb-6 italic">News Archive</h1>
            <p className="font-body text-lg text-on-surface-variant max-w-2xl">
              Latest dispatches, policy updates, and insights from the campaign trail.
            </p>
          </div>
        </section>
          ...
          <div className="max-w-screen-2xl mx-auto px-6">
            <div className="grid gap-16">
              {newsItems.map((item) => (
                <article
                  key={item.id}
                  className="grid md:grid-cols-3 gap-8 group hover:shadow-editorial p-8 rounded-lg transition-all duration-300 hover:bg-surface-container-low"
                >
                  <div className="md:col-span-1 overflow-hidden bg-surface-container-high rounded-lg hover-scale">
                    <img
                      src={item.image}
                      alt={item.title}
                  ...
                  <div className="md:col-span-2 flex flex-col">
                    <span className="font-label text-[10px] font-bold uppercase tracking-[0.3em] text-on-surface-variant mb-3">
                      {item.category} • {item.date} • {item.readTime}
                    </span>
                    <h3 className="font-headline text-2xl md:text-3xl mb-4 group-hover:text-secondary transition-colors duration-300">
                      {item.title}
                    </h3>
                    ...
                    <Link
                      href={`/news/${item.id}`}
                      className="font-label text-xs font-bold uppercase tracking-widest inline-flex items-center gap-2 hover:translate-x-1 transition-transform duration-300 hover:text-secondary w-fit"
                    >
                      Read Full Story{' '}
